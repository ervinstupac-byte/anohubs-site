...
const insightData = [
    // NAPOMENA: Sadržaj modula je očišćen od vanjskog <div class="article-container"> taga.
...
function renderInsights(data) {
    ...
    data.forEach(item => {
        ...
        const previewText = (articleIntro || tempDiv.textContent).substring(0, 100).replace(/\s+/g, ' ').trim();
...
function openModal(moduleId) {
    ...
    if (article && contentWrapper) {
        // KRITIČNO: Ubacujemo article.content OMOTANO u article-container
        contentWrapper.innerHTML = `<div class="article-container">${article.content}</div>`;
    } else {
        contentWrapper.innerHTML = '<div class="article-container text-center"><h1 class="text-red-600">ERROR: Article Content Not Found.</h1></div>';
    }
